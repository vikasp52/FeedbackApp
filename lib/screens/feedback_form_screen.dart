...
class _FeedbackFormScreenState extends State<FeedbackFormScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Padding(
        padding: const EdgeInsets.all(8.0),
        child: StreamBuilder<FeedbackFormModel>(
            stream: feedbackFormBloc.feedbackFormStream,
            builder: (context, snapshot) {
              print("UI Data1: ${snapshot.connectionState}");
              print("UI Data: ${snapshot.data}");
              if (snapshot.hasData) {
                FeedbackFormModel formData = snapshot.data;
                //print("formData: ${formData.gender_choice.toString()}");
                    ...
                    child: Column(
                      children: <Widget>[
                        TextFieldWidget(
                          ...
                          onChanged: feedbackDataBloc.nameSink.add,
                        ),
                        TextFieldWidget(
                          hintText: formData.email,
                          stream: feedbackDataBloc.emailStream,
                          onChanged: feedbackDataBloc.emailSink.add,
                        ),
                        TextFieldWidget(
                          ...
                          onChanged: feedbackDataBloc.phoneNoSink.add,
                        ),
                        TextFieldWidget(
                          hintText: formData.age,
                          keyboardType: TextInputType.number,
                          stream: feedbackDataBloc.ageStream,
                          onChanged: feedbackDataBloc.ageSink.add,
                        ),
